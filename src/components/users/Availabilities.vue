<script>
import { onMounted, ref } from 'vue';
import axios from '../../Axios';
import { useRoute } from 'vue-router';

export default {
  setup() {
    const availabilities = ref([])
    const status = ref(0)
    const page = ref(1)
    const user = ref()
    const route = useRoute()
    const serviceId = route.params.serviceId

    const getAvailabilities = async () => {
      try {
        const response = await axios.get(`/services/${serviceId}/availabilities`, {
          headers: user.value,
          params: {
            page: page.value
          }
        })

        if (response.status === 200) {
          status.value = response.status
          availabilities.value = response.data.availabilities
        }
      } catch (error) {
        console.error(error);
      }
    }

    onMounted(() => {
      const authHeaders = localStorage.getItem('authHeaders')
      if (authHeaders) {
        user.value = JSON.parse(authHeaders)
        getAvailabilities()
      }
    })

    return {
      user, availabilities, status, page
    }
  }
}
</script>

<template>
  <div v-if="user" class="container p-3">
    <router-link to="/services" title="Atrás">
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-arrow-left-circle"
        viewBox="0 0 16 16">
        <path fill-rule="evenodd"
          d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8m15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-4.5-.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5z" />
      </svg>
    </router-link>
    <div class="text-center mb-3">
      <h1>Mi Disponibilidad</h1>
    </div>
    <div v-if="status === 200" class="table-responsive text-center">
      <table class="table table-hover">
        <thead class="table-info">
          <tr>
            <th>Semana</th>
            <th>Fechas</th>
            <th>Servicio ID</th>
            <th>Confirmado</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="avilability in availabilities">
            <td>{{ avilability.week }}, {{ avilability.year }}</td>
            <td>Desde: {{ avilability.from }} | Hasta: {{ avilability.to }}</td>
            <td>{{ avilability.service_id }}</td>
            <td>
              <template v-if="avilability.confirmed">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                  class="bi bi-check-circle text-success" viewBox="0 0 16 16">
                  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
                  <path
                    d="m10.97 4.97-.02.022-3.473 4.425-2.093-2.094a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05" />
                </svg>
              </template>
              <template v-else>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                  class="bi bi-x-circle text-danger" viewBox="0 0 16 16">
                  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
                  <path
                    d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708" />
                </svg>
              </template>
            </td>
            <td>
              <template v-if="avilability.confirmed">
                <button type="button" class="btn btn-outline-info">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-eye"
                    viewBox="0 0 16 16">
                    <path
                      d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z" />
                    <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0" />
                  </svg>
                </button>
              </template>
              <template v-else>
                <button type="button" class="btn btn-outline-success">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                    class="bi bi-pencil" viewBox="0 0 16 16">
                    <path
                      d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325" />
                  </svg>
                </button>
              </template>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <div v-if="status !== 200" class="text-center">Esperando...</div>
  </div>
</template>